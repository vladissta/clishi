...
simulate_fun_calc_new <-function(x){
...
means_distribution_df <- function(samples_df){
  samples_df |>
    group_by(sample_ID) |>
...
true_mean_calc <- function(dist, params_list){
  req(dist)
  
  switch(dist,
         norm = {params_list$mean},
         unif = {(params_list$b + params_list$a) / 2},
         exp = {1/(params_list$rate)})
}


descriptive_stats <- function(data_trial, true_mean){
  eda_df <- 
    tibble(
      'True mean' = true_mean,
      'Mean of the means' = mean(data_trial$mu),
      'Error' = sqrt(mean((data_trial$mu-true_mean)^2)),
